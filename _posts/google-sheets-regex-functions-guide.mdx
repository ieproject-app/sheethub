...
If the job is simply splitting a value at a known delimiter, the [Google Sheets text functions](/blog/split-text-functions-google-sheets) guide may be simpler. REGEX is most useful when the separator varies or the text has a recognizable pattern rather than one fixed character.
