...
tags: ["excel", "tutorial", "stockhistory", "historical-data", "dynamic-arrays"]
...
**The function returns an error:** Check the ticker spelling, date order, and whether `start_date` is a valid Excel date. Test the formula with `DATE(year,month,day)` rather than a text date that may be interpreted differently by regional settings.

**The output is too wide:** Remove properties you do not need. A chart of closing prices normally needs only date and close, not every available field.

**The output shows `#SPILL!`:** Inspect every cell below and to the right of the formula. Clear accidental text, formulas, merged cells, or table boundaries that block the result.

**The price looks different from another website:** Sources can use different market conventions, timing, adjustments, or delayed values. Compare the instrument, interval, and property, then verify important figures against a trusted source.

**The formula does not work for a user:** Check the user's Microsoft 365 eligibility and connection. A workbook that contains STOCKHISTORY may not behave identically for every account or Excel environment.
