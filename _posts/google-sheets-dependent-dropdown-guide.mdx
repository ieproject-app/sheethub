...
- **The second list is empty:** Confirm the parent text matches the source category.
- **The formula shows `#N/A`:** Keep `IFNA` and check whether the category has matches.
- **The list stops early:** Expand the validation source range beyond the first helper cell.
- **The formula shows `#REF!`:** Clear cells below the helper formula.
- **Duplicates or row conflicts appear:** Keep `UNIQUE`, check spelling, and give each row its own spill area.
