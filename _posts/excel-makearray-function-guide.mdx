...
The formula builds 9 rows and 9 columns. Each cell multiplies its row number by its column number, so cell (3, 4) gets `3*4` = 12 and cell (7, 2) gets 14. Change `9` to `12` and the table grows with one edit instead of 144 copied formulas.
...
`RANDBETWEEN` is volatile, so the whole grid recalculates every time the sheet changes. If you need a stable snapshot, copy the result and paste it as values with <Kbd>Ctrl</Kbd> + <Kbd>Shift</Kbd> + <Kbd>V</Kbd> (Windows) or <Kbd>Cmd</Kbd> + <Kbd>Shift</Kbd> + <Kbd>V</Kbd> (Mac).
...
In every case, the result is a dynamic array that spills like [FILTER or SEQUENCE](/blog/excel-dynamic-array-functions-guide). The dimensions stay fixed, while the values recalculate whenever the inputs change.
