...
tags: ["google-sheets", "named-functions", "formulas", "reusable-formulas", "tutorial"]
...
The important detail is that `amount` and `rate` are placeholders, not fixed cell addresses. The same definition can therefore be used for row 2, row 20, or a value typed directly into a formula.

Choose names that describe the inputs. `amount` and `rate` are easier to understand than `x` and `y,` especially when someone opens the named-function editor months later. Keep the description specific: mention the expected input and the result, and state whether a percentage must be entered as a decimal such as `0.08`.
...
1. **Placeholder mismatch:** Every placeholder used in the formula definition must be declared, and each call must supply the required arguments. Rename or add the missing placeholder in the editor.
2. **Wrong argument order:** `GROSS_UP(rate,amount)` produces a plausible-looking but incorrect result. Keep the function description and examples in the same order as the definition.
3. **Range-size mismatch:** Functions such as `FILTER` require the data and condition ranges to align. Check the starting row and ending row on both ranges.
4. **Table-reference assumptions:** Structured table references from other spreadsheet tools may not behave like ordinary A1 ranges in Sheets. Replace them with explicit ranges while testing, then confirm the result in the real workbook.
5. **Circular references:** A named function that refers to a range containing its own output can create a circular dependency. Move the output range or pass only the input range.
6. **Name collisions:** A function name that resembles a cell address, named range, or built-in function can cause confusion or a parse error. Choose a more descriptive name.

When debugging, first replace the named-function call with its full formula and test the pieces separately. Once the full formula works, compare each placeholder with the corresponding expression in the definition.
