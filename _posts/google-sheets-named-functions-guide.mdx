...
If the result is meant to feed a report, an [advanced QUERY in Google Sheets](/blog/query-function-google-sheets-guide) may be a better fit when grouping, labeling, or sorting is the main task. A named function is most useful when the same piece of logic appears in several formulas.
...
Use a named function when the same multi-step calculation appears in multiple places and its inputs can be described clearly. Use `LET` when the logic is local to one formula and you mainly need readable intermediate variables. The [LET guide for Google Sheets](/blog/excel-let-function-guide) is a useful comparison for that pattern.
