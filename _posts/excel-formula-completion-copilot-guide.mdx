...
tags: ["excel", "copilot", "formula-completion", "news", "update", "tutorial"]
...
1. **The function matches the goal.** If the column is named `Quarter`, the suggestion should calculate a quarter, not concatenate text.
2. **The references point at the right cells.** A formula that uses `C2` should be in a row where column C holds the source value.
3. **The range makes sense.** The proposal should cover the rows you expect and stop where the data stops.
4. **The explanation matches the formula.** If the tooltip says "calculate quarter from invoice date," the formula should do exactly that.
...
- **No qualifying license.** The feature requires a Microsoft 365 Copilot license. A standard Microsoft 365 subscription may not show suggestions at all.
- **Preview or channel restrictions.** The feature may be limited to specific channels such as Current Channel or Insider builds, depending on the rollout stage.
- **Ambiguous headers.** A column named `Data` gives Excel little to work with. Clear, specific headers produce better suggestions.
- **Wrong or partial ranges.** If the table has blank rows or mixed types, the proposal may reference too many cells or skip rows.
- **Formula displayed as text.** When a cell shows the formula text instead of a result, the cell is formatted as text or the formula starts with an apostrophe. Change the format or remove the apostrophe and re-enter the formula.
- **Sensitive data in context.** The suggestion is generated from the sheet context. Do not use formula completion with data that must not leave your organization, and follow your administrator's Copilot policies.
