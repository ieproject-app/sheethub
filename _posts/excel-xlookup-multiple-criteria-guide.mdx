...
This guide compares three approaches: Boolean multiplication inside XLOOKUP, a concatenated lookup key, and a helper key column. The first is compact, the second keeps the logic in one formula, and the third is usually easiest for a team to audit.
...
This is the most transparent method. A reviewer can inspect the key, filter it, and identify spaces or inconsistent values without unpacking an array formula. It also makes troubleshooting easier when a user reports `Not found`.
...
All three methods use XLOOKUP's first-match behavior. None of them automatically resolves duplicate rows. Decide whether duplicates are valid, report them with `COUNTIF`, or add another condition such as an effective date.
