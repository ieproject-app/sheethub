...
If Power Query detects a wrong data type, you can correct it later. At this stage, decide which columns should remain attached to each value.
...
This option saves selection work, but it requires more care. Any column that is not an identifier will be treated as a measure. If you later add a Notes or Manager column and forget to classify it as an identifier, that text may appear in the unpivoted Value column.
