...
Google Sheets has two very different kinds of import. `IMPORTRANGE` moves cells between spreadsheets you control. `IMPORTHTML`, `IMPORTXML`, and `IMPORTDATA` read public content from the web. The formulas look related, but the source, failure modes, and maintenance work are different.
...
The source owner or editor must authorize the connection the first time. The [Google Sheets QUERY guide](/blog/google-sheets-query-advanced-guide) can then filter or reshape the imported range.
...
You can also filter an imported range directly:
...
This repeats the import and may fetch twice. Prefer one import in a staging tab, then analyze the local result with the [advanced Google Sheets QUERY guide](/blog/google-sheets-query-advanced-guide).
