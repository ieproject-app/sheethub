...
In Google Sheets, QUERY can do more than select and filter rows. PIVOT, LABEL, and FORMAT reshape the result for a report. SELECT, WHERE, ORDER BY, and LIMIT retrieve rows; these advanced clauses control how the output is organized. If the report needs interactive field controls instead, compare it with our [Google Sheets QUERY Pivot comparison with pivot tables](/blog/google-sheets-pivot-tables-guide).
