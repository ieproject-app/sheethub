...
This Google Sheets QUERY Pivot guide shows how PIVOT, LABEL, and FORMAT turn a basic query into a presentation-ready report. SELECT, WHERE, ORDER BY, and LIMIT retrieve rows; these advanced clauses shape the result for reporting. If the report needs interactive field controls instead, compare it with our [Google Sheets QUERY Pivot comparison with pivot tables](/blog/google-sheets-pivot-tables-guide).
...
PIVOT is the formula-based cousin of a pivot table: a pivot table lets you drag fields around interactively, while QUERY PIVOT recalculates when the source changes. For a practical comparison with filtered totals, see this guide to [summing filtered rows in Google Sheets](/blog/google-sheets-sum-filtered-data-custom-criteria).
...
- Keep QUERY ranges tight when combining them with [Google Sheets date functions for report filters](/blog/google-sheets-date-functions-guide), and avoid pulling unused columns into a large query.
